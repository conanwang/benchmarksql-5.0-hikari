...
public class jTPCCTerminal implements jTPCCConfig, Runnable
{
    ...
    public jTPCCTerminal
      (String terminalName, int terminalWarehouseID, int terminalDistrictID,
       ...
       int deliveryWeight, int stockLevelWeight, int numWarehouses, int limPerMin_Terminal, jTPCC parent, HikariDataSource ds) throws SQLException
    {
...
label: for(int i = 0; (i < numTransactions || numTransactions == -1) && !stopRunning; i++)
	{ 
        /*add by conawnang*/
        while (true) {
            try {
                //log.warn("get conn handler");
                this.db = new jTPCCConnection(conn, dbType); //add by conanwang
                //log.warn("db is" + db);
                break;
             } catch (Exception e) {
                try {
