...
public class jTPCC implements jTPCCConfig
{
	...
	if(databaseDriverLoaded)
	{
		...
		try
		{
			...
			hikariConfig.setUsername(iUser);
			hikariConfig.setPassword(iPassword);
			...
			(terminalName, terminalWarehouseID, terminalDistrictID,
			 ...
			 deliveryWeightValue, stockLevelWeightValue, numWarehouses, limPerMin_Terminal, this, ds);
			...
			for(int i = 0; i < terminals.length; i++)
			    (new Thread(terminals[i])).start(); //这里开始启动所有线程
	...
	if(terminalsStarted == 0)
	{
	    ...
	    log.warn("All terminals finished executing " + sessionEnd);
	    endReport();
